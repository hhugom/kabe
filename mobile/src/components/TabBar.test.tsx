...
// Fabricates a minimal BottomTabBarProps object. TabBar's public interface *is*
// BottomTabBarProps, so we drive it exactly the way React Navigation would.
...
describe('TabBar', () => {
  ...
  it('does not render a center Start/Resume button', async () => {
    // Center action retired: Start-a-Session moved back to Home's hero card
    // (see docs/conventions/navigation-surface.md § Home Start hero). The tab
    // bar is a strict 3-cell flat strip.
    const { queryByTestId, queryByText } = await render(<TabBar {...makeProps()} />);
    expect(queryByTestId('tab-touch-Start')).toBeNull();
